...
Equity of outcomes prob
```{r}

eq_out_prob <- data.frame(group=rownames(phys_students_prob),
             q = phys_students_prob$q-phys_students_prob$q[[1]],
             se = (phys_students_prob$test$sigma))
             up = phys_students_prob$q-phys_students_prob$UL+)
...
phys_courses_prob$semester <- factor(c("All", "All", "1st", "1st", "2nd", "2nd", "3rd", "3rd", "1st", "1st", "2nd", "2nd"))
...
studentplot <- ggplot(phys_students_prob, aes(x=demo, y=q, fill=instruction)) + geom_bar(stat = "identity", position= "dodge") + geom_errorbar(aes( ymax= UL, ymin=LL), position="dodge") +
  theme(legend.position = "bottom", axis.title.x=element_blank(), axis.text.x=element_text(angle=90), axis.ticks.x=element_blank(),panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"),text = element_text(size=16, color = "black")) + ylab("DFW probability") + facet_grid(.~firstgen, scales = "free", space = "free")

plot(studentplot)
ggsave("students.png", plot= last_plot(), dpi=300, width = 7, height = 6, units = "in", device = "png")

```

equity of outcome plot
```{r}
eq_out_prob$demo <- factor(c("White Men", "White Men", "White Women", "White Women", "Men of Color", "Men of Color", "Women of Color", "Women of Color", "White Men", "White Men", "White Women", "White Women", "Men of Color", "Men of Color", "Women of Color", "Women of Color"))
eq_out_prob$Race <- factor(c("White", "White", "White", "White", "POC", "POC", "POC", "POC", "White", "White", "White", "White", "POC", "POC", "POC", "POC"))
eq_out_prob$instruction <- factor(c("traditional", "LA", "traditional", "LA", "traditional", "LA", "traditional", "LA", "traditional", "LA", "traditional", "LA", "traditional", "LA", "traditional", "LA"))
eq_out_prob$firstgen <- factor(c("non-firstgen", "non-firstgen", "non-firstgen", "non-firstgen", "non-firstgen", "non-firstgen", "non-firstgen", "non-firstgen", "firstgen", "firstgen", "firstgen", "firstgen", "firstgen", "firstgen", "firstgen", "firstgen"))

eq_out_plot <- ggplot(eq_out_prob, aes(x=demo, y=q, fill=firstgen)) + geom_bar(stat = "identity", position= "dodge") + 
  theme(legend.position = "bottom", axis.title.x=element_blank(), axis.text.x=element_text(angle=90), axis.ticks.x=element_blank(),panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"),text = element_text(size=16, color = "black")) + ylab("DFW probability") + facet_grid(.~instruction, scales = "free", space = "free")
...
desc <- red_phys %>% group_by(Number) %>% summarise(N = length(DDFW),
             ...
             firstgen_mean = mean(firstgen),
             firstgen_N = sum(firstgen),
