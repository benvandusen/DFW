...
intro_mod_final1 <- (DDFW ~ FG_Status*LA + Female*LA + nonwhite*LA + FG_Status*nonwhite + NULLGender*LA + (1|courID))
intro_m_final1 <- glmer(intro_mod_final, data=red_introPhys,family = binomial)
intro_mod_final <- (DDFW ~ FG_Status*LA + Female*LA + nonwhite*LA + FG_Status*nonwhite + NULLGender + (1|courID))
...
```{r}
for_descriptives_df <- red_introPhys[c("DDFW","FG_Status","Female","algebra","nonwhite","LA","race_Non-Resident Alien","race_Unknown")]
descriptives_algebra <- for_descriptives_df %>% group_by(FG_Status, Female, algebra, nonwhite, LA) %>%
  summarise(N = length(DDFW),
            DDFW= sum(DDFW))
descriptives_all <- for_descriptives_df %>% group_by(FG_Status, Female, nonwhite, LA) %>%
  summarise(N = length(DDFW),
            DDFW= sum(DDFW))

descriptives_all_NRA <- for_descriptives_df %>% group_by(FG_Status, Female, nonwhite, LA,`race_Non-Resident Alien`) %>%
  summarise(N = length(DDFW),
            DDFW= sum(DDFW))
...
descriptives_all_NRA <- for_descriptives_df %>% group_by(FG_Status, Female, nonwhite, LA,race_Unknown) %>%
  summarise(N = length(DDFW),
            DDFW= sum(DDFW))
