...
reduced <- as.data.frame(dplyr::select(DFW, TERM, Subject, Number,courID, Section, instructor,FG_Status, gender, race, dependent_income, LA, DDFW))
...
intro_mod_final <- (DDFW ~ FG_Status*LA + Female*LA + nonwhite*LA + FG_Status*nonwhite + (1|courID))
...
```{r}
eq_ind_plot <- ggplot(eq_ind_prob, aes(x=reorder(demo, new.order=c("White Men","White Women", "Men of Color", "Women of Color")), y=qdiff, fill=FG_Status)) + geom_bar(stat = "identity", position= "dodge") + 
