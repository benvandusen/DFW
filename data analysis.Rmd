...
```{r}
for_descriptives_df <- red_introPhys[c("DDFW","FG_Status","Female","algebra","nonwhite","LA","race_Non-Resident Alien")]
...
red_introPhys$race_Asian
#write.csv(descriptives_all, file="descriptives_table.csv")
write.csv(descriptives_all_NRA, file="descriptives_table_NRA.csv")

temp <- MIdata[[1]]
mytable <- xtabs(~`race_Non-Resident Alien`+ `race_American Indian/Alaskan Native` + race_Asian, data=red_introPhys)
ftable(mytable)
