...
phys_students_prob$firstgen <- factor(c("Non-FG", "Non-FG", "Non-FG", "Non-FG", "Non-FG", "Non-FG", "Non-FG", "Non-FG", "FG", "FG", "FG", "FG", "FG", "FG", "FG", "FG"))
```
...
eq_out_prob <- data.frame(group=rownames(phys_students_prob),
             ...
             firstgen = phys_students_prob$firstgen,
...
eq_ind_prob <- eq_ind_prob %>% group_by(demo,firstgen) %>% mutate(qdiff = q - q[[1]])
eq_ind_prob <- eq_ind_prob %>% group_by(demo,firstgen) %>% mutate(seUL = ((seL[[1]])^2+(seU)^2)^0.5)
eq_ind_prob <- eq_ind_prob %>% group_by(demo,firstgen) %>% mutate(seLL = ((seU[[1]])^2+(seL)^2)^0.5)
...
eq_out_plot <- ggplot(eq_out_prob, aes(x=reorder(demo_fg, new.order=c(" Men of Color", " White Men", "Men of Color", " Women of Color", " White Women", "Women of Color", "White Women")), y=q, fill=firstgen)) + geom_bar(stat = "identity", position= "dodge") + coord_flip()+
...
```{r}
eq_ind_plot <- ggplot(eq_ind_prob, aes(x=reorder(demo, new.order=c("White Men","White Women", "Men of Color", "Women of Color")), y=qdiff, fill=firstgen)) + geom_bar(stat = "identity", position= "dodge") + 
  theme(legend.position = "bottom", axis.title.x=element_blank(), axis.text.x=element_text(angle=90), axis.ticks.x=element_blank(),panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"),text = element_text(size=16, color = "black")) + ylab("DFW probability")
...
desc_firstgen <- freq(red_phys$firstgen)
desc_gen
desc_race
desc_firstgen
